      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-download" class="form-horizontal">
		  <div class="form-group">
            <label class="col-sm-2 control-label"><?php echo $entry_vendor_name; ?></label>
            <div class="col-sm-10">
			  <select name="vendor" class="form-control">
				<option value="0"><?php echo $text_none; ?></option>
                <?php foreach ($vendors as $vds) { ?>
					<?php if ($vds['vendor_id'] == $vendor) { ?>
					<option value="<?php echo $vds['vendor_id']; ?>" selected="selected"><?php echo $vds['vendor_name']; ?></option>
					<?php } else { ?>
					<option value="<?php echo $vds['vendor_id']; ?>"><?php echo $vds['vendor_name']; ?></option>
					<?php } ?>
               	<?php } ?>
              </select>
            </div>
...
$('#button-upload').on('click', function() {
	...
	timer = setInterval(function() {
		if ($('#form-upload input[name=\'file\']').val() != '') {
			...
			$.ajax({
				url: 'index.php?route=catalog/mvd_download/upload&token=<?php echo $token; ?>',
				type: 'post',		
				dataType: 'json',
