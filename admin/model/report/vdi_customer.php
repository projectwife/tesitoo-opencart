<?php
class ModelReportVDICustomer extends Model {
	public function getTotalCustomersByDay() {
		...
		$query = $this->db->query("SELECT COUNT(DISTINCT (c.customer_id)) AS total, HOUR(c.date_added) AS hour FROM `" . DB_PREFIX . "customer` c LEFT JOIN `" . DB_PREFIX . "order` o ON (c.customer_id = o.customer_id) LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id ) WHERE DATE(c.date_added) = DATE(NOW()) AND op.vendor_id = '" . (int)$this->user->getVP() . "' GROUP BY HOUR(c.date_added) ORDER BY c.date_added ASC");
	...
	public function getTotalCustomersByWeek() {
		...
		$query = $this->db->query("SELECT COUNT(DISTINCT (c.customer_id)) AS total, c.date_added FROM `" . DB_PREFIX . "customer` c LEFT JOIN `" . DB_PREFIX . "order` o ON (c.customer_id = o.customer_id) LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id ) WHERE DATE(c.date_added) >= DATE('" . $this->db->escape(date('Y-m-d', $date_start)) . "') AND op.vendor_id = '" . (int)$this->user->getVP() . "' GROUP BY DAYNAME(c.date_added)");
	...
	public function getTotalCustomersByMonth() {
		...
		$query = $this->db->query("SELECT COUNT(DISTINCT (c.customer_id)) AS total, c.date_added FROM `" . DB_PREFIX . "customer` c LEFT JOIN `" . DB_PREFIX . "order` o ON (c.customer_id = o.customer_id) LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id ) WHERE DATE(c.date_added) >= '" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "' AND op.vendor_id = '" . (int)$this->user->getVP() . "' GROUP BY DATE(c.date_added)");
	...
	public function getTotalCustomersByYear() {
		...
		$query = $this->db->query("SELECT COUNT(DISTINCT (c.customer_id)) AS total, c.date_added FROM `" . DB_PREFIX . "customer` c LEFT JOIN `" . DB_PREFIX . "order` o ON (c.customer_id = o.customer_id) LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id ) WHERE YEAR(c.date_added) = YEAR(NOW()) AND op.vendor_id = '" . (int)$this->user->getVP() . "' GROUP BY MONTH(c.date_added)");
