...
class ModelSaleVDIOrder extends Model {
    ...
    public function getOrdersByOrderProductStatus($data = array()) {
		$sql = "SELECT DISTINCT o.order_id, op.order_status_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = op.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, ";

		$sql .= " (SELECT SUM(op.total) FROM " . DB_PREFIX . "order_product op WHERE op.order_id = o.order_id AND op.vendor_id = '" . (int)$this->user->getVP() . "' LIMIT 1) AS total,";
		$sql .= " (SELECT os.cost FROM " . DB_PREFIX . "order_shipping os WHERE os.order_id = o.order_id AND os.vendor_id = '" . (int)$this->user->getVP() . "' LIMIT 1) AS shipping_cost,";

		$sql .= " o.currency_code, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id)";
		...
		if (isset($data['filter_order_status'])) {
			...
			foreach ($order_statuses as $order_status_id) {
				$implode[] = "op.order_status_id = '" . (int)$order_status_id . "'";
			}
	...
	public function getTotalOrders($data = array()) {
		$sql = "SELECT COUNT(DISTINCT o.order_id, op.order_status_id) AS total FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id)";
