<?php
class ModelCatalogMVDDownload extends Model {
	public function addDownload($data) {
		...
		//mvds
		$this->db->query("INSERT INTO " . DB_PREFIX . "download SET filename = '" . $this->db->escape($data['filename']) . "', vendor_id = '" . (int)$data['vendor'] . "', mask = '" . $this->db->escape($data['mask']) . "', date_added = NOW()");
		//mvde
	...
	public function editDownload($download_id, $data) {
		...
		//mvds
		$this->db->query("UPDATE " . DB_PREFIX . "download SET filename = '" . $this->db->escape($data['filename']) . "', vendor_id = '" . (int)$data['vendor'] . "', mask = '" . $this->db->escape($data['mask']) . "' WHERE download_id = '" . (int)$download_id . "'");
		//mvde
	...
	public function getDownloads($data = array()) {
		//mvds
		$sql = "SELECT * FROM " . DB_PREFIX . "download d LEFT JOIN " . DB_PREFIX . "download_description dd ON (d.download_id = dd.download_id) LEFT JOIN " . DB_PREFIX . "vendors vds ON (d.vendor_id = vds.vendor_id) WHERE dd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		//mvde
