<?php
class ModelCatalogMVDOption extends Model {
	public function addOption($data) {
		$this->event->trigger('pre.admin.add.option', $data);
		
		//mvds
		$this->db->query("INSERT INTO `" . DB_PREFIX . "option` SET type = '" . $this->db->escape($data['type']) . "', vendor_id = '" . (int)$data['vendor'] . "', sort_order = '" . (int)$data['sort_order'] . "'");
		//mvde
	...
	public function editOption($option_id, $data) {
		...
		//mvds
		$this->db->query("UPDATE `" . DB_PREFIX . "option` SET type = '" . $this->db->escape($data['type']) . "', vendor_id = '" . (int)$data['vendor'] . "', sort_order = '" . (int)$data['sort_order'] . "' WHERE option_id = '" . (int)$option_id . "'");
		//mvde
	...
	public function getOptions($data = array()) {
		//mvds
		$sql = "SELECT * FROM `" . DB_PREFIX . "option` o LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) LEFT JOIN " . DB_PREFIX . "vendors vds ON (o.vendor_id = vds.vendor_id) WHERE od.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		//mvde
