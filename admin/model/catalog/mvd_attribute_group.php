<?php
class ModelCatalogMVDAttributeGroup extends Model {
	public function addAttributeGroup($data) {
		$this->event->trigger('pre.admin.add.attribute.group', $data);
		
		//mvds
		$this->db->query("INSERT INTO " . DB_PREFIX . "attribute_group SET sort_order = '" . (int)$data['sort_order'] . "', vendor_id = '" . (int)$data['vendor'] . "'");
		//mvde
	...
	public function editAttributeGroup($attribute_group_id, $data) {
		...
		//mvds
		$this->db->query("UPDATE " . DB_PREFIX . "attribute_group SET sort_order = '" . (int)$data['sort_order'] . "', vendor_id = '" . (int)$data['vendor'] . "' WHERE attribute_group_id = '" . (int)$attribute_group_id . "'");
		//mvde
	...
	public function getAttributeGroups($data = array()) {
		//mvds
		$sql = "SELECT * FROM " . DB_PREFIX . "attribute_group ag LEFT JOIN " . DB_PREFIX . "attribute_group_description agd ON (ag.attribute_group_id = agd.attribute_group_id) LEFT JOIN " . DB_PREFIX . "vendors vds ON (ag.vendor_id = vds.vendor_id) WHERE agd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		//mvde
