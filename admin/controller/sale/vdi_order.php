...
class ControllerSaleVDIOrder extends Controller {
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('sale/vdi_order/add', 'token=' . $this->session->data['token'], 'SSL');
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				'edit'          => $this->url->link('sale/vdi_order/edit', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL'),
			);
		}
