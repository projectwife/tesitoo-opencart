	...
	public function addVendorSignUp($data) {
		...
		if ($this->request->post['signup_plan']) {
			...
			if ($this->config->get('mvd_signup_auto_approval')) {
				if ($signup_plan[1] == '4' && $data['payment_method'] == '1') {
					...
					$this->db->query("INSERT INTO `" . DB_PREFIX . "signup_fee_history` SET user_id = '" . (int)$user_id . "', signup_fee = '" . (float)$signup_plan[4] . "', signup_plan = '" . $this->db->escape($data['hsignup_plan']) . "', vendor_name = '" . $this->db->escape($data['company']) . "', username = '" . $this->db->escape($data['username']) . "', commission_type = '" . (int)$signup_plan[1] . "', status = '5', user_date_start = Now(), user_date_end = Now() + INTERVAL " . $signup_plan[3] . " MONTH, date_added = NOW()");
				} elseif($signup_plan[1] == '5' && $data['payment_method'] == '1') {
					$this->db->query("UPDATE " . DB_PREFIX . "user SET status = '5', folder = '" . $this->db->escape($data['username']) . "', vendor_permission = '" . (int)$vendor_id . "', cat_permission = '" . serialize($this->config->get('signup_category')) . "', store_permission = '" . serialize($this->config->get('signup_store')) . "' , user_date_start = Now(), user_date_end = Now() + INTERVAL " . $signup_plan[3] . " YEAR WHERE user_id = '" . (int)$user_id . "'");
					$this->db->query("INSERT INTO `" . DB_PREFIX . "signup_fee_history` SET user_id = '" . (int)$user_id . "', signup_fee = '" . (float)$signup_plan[4] . "', signup_plan = '" . $this->db->escape($data['hsignup_plan']) . "', vendor_name = '" . $this->db->escape($data['company']) . "', username = '" . $this->db->escape($data['username']) . "', commission_type = '" . (int)$signup_plan[1] . "', status = '5', user_date_start = Now(), user_date_end = Now() + INTERVAL " . $signup_plan[3] . " YEAR, date_added = NOW()");
				} elseif ($signup_plan[1] == '4' && $data['payment_method'] == '0') {
					...
					$this->db->query("INSERT INTO `" . DB_PREFIX . "signup_fee_history` SET user_id = '" . (int)$user_id . "', signup_fee = '" . (float)$signup_plan[4] . "', signup_plan = '" . $this->db->escape($data['hsignup_plan']) . "', vendor_name = '" . $this->db->escape($data['company']) . "', username = '" . $this->db->escape($data['username']) . "', commission_type = '" . (int)$signup_plan[1] . "', status = '" . (int)$this->config->get('mvd_bank_order_status') . "', user_date_start = Now(), user_date_end = Now() + INTERVAL " . $signup_plan[3] . " MONTH, date_added = NOW()");
				} elseif ($signup_plan[1] == '5' && $data['payment_method'] == '0') {
					$this->db->query("UPDATE " . DB_PREFIX . "user SET status = '" . (int)$user_status . "', folder = '" . $this->db->escape($data['username']) . "', vendor_permission = '" . (int)$vendor_id . "', cat_permission = '" . serialize($this->config->get('signup_category')) . "', store_permission = '" . serialize($this->config->get('signup_store')) . "' , user_date_start = Now(), user_date_end = Now() + INTERVAL " . $signup_plan[3] . " YEAR WHERE user_id = '" . (int)$user_id . "'");
					$this->db->query("INSERT INTO `" . DB_PREFIX . "signup_fee_history` SET user_id = '" . (int)$user_id . "', signup_fee = '" . (float)$signup_plan[4] . "', signup_plan = '" . $this->db->escape($data['hsignup_plan']) . "', vendor_name = '" . $this->db->escape($data['company']) . "', username = '" . $this->db->escape($data['username']) . "', commission_type = '" . (int)$signup_plan[1] . "', status = '" . (int)$this->config->get('mvd_bank_order_status') . "', user_date_start = Now(), user_date_end = Now() + INTERVAL " . $signup_plan[3] . " YEAR, date_added = NOW()");
				} else {
			...
			if ($this->config->get('mvd_signup_auto_approval')) {
				...
				if ($signup_plan[1] == '4' || $signup_plan[1] == '5') {
					$message .= $this->language->get('text_login') . "<br><br>";
				} else {
			...
			if ($signup_plan[1] == '4') {
				$message .= $this->language->get('text_signup_expire_date') . date('Y-m-d', strtotime("+" . $signup_plan[3] . "month")) . "<br>";
			} elseif($signup_plan[1] == '5') {
			...
			$signup_plan = $this->request->post['signup_plan'];
